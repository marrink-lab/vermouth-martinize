...
def add_virtual_sites(molecule, prefix, atomname='BB'):
    """
    ...
    bead. It also copies the *reference* to the poition array, so the virtual
    ...
    charge group, with "CA" as atomname, and a charge of 0.
    ...
    new_charge_group = max(node['charge_group'] for node in molecule.nodes.values())
    for node_id, atom in molecule.nodes(data=True):
        if atom.get('atomname') == atomname:
            new_node_id += 1
            new_charge_group += 1
            ...
            virtual_sites.append(Interaction(
                ...
                parameters=['1'],
                meta={'go_vs': True, 'comment': 'virtual go site'},
            ))
