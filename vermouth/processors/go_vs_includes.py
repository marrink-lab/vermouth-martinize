...
"""
Add the include stratements required to use te VirtualGoSite model.
...
The processor defined in this modeule does not generate the Go potentials.
Instead, they let the potential being generated by a third party program. The
third party program generate the virtual sites, and the exclusions as ITP files
to be included in the right place in the protein ITP file. The processor adds
the include statements where they are needed: at the end of the `[ atoms ]`,
`[ virtual_sitesn ]`, and `[ exclusions ]` section. To do so, the processor
adds the required lines in the "post_section_lines" meta attribute of the
molecules. This meta attribute is read by :func:`vermouth.gmx.itp.write_molecule_itp`.

The include files are called "<moltype>_<section>_VirtGoSite.itp".
"""
...
class GoVirtIncludes(Processor):
    ...
    """
    def __init__(self, sections=('atoms', 'virtual_sitesn', 'exclusions')):
