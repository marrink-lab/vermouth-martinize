...
def calculate_contact_map(mol):
    ...
    for node in mol.nodes:
        try:
            ...
            res_serial.append(mol.nodes[node]['_res_serial'])
    ...
    nresidues = len(list(set(res_serial)))
    ...
    res_cogs = np.stack([np.stack(coords[np.where(res_serial == i)[0][0]:
                                         np.where(res_serial == i)[0][-1]]).mean(axis=0)
                         for i in range(nresidues)])
    res_dists = res2atom(cdist(res_cogs, res_cogs), res_serial, nresidues)
    ...
    # resnames = np.array(resnames)[np.unique(res_serial, return_index=True)[1]]
    resids = np.array(residues)[np.unique(res_serial, return_index=True)[1]]
    chains = np.array(chains)[np.unique(res_serial, return_index=True)[1]]
    ...
    #                        f"{int(i1 + 1):5d}  {resnames[i1]:3s} {chains[i1]:1s} {int(resids[i1]):4d}    "
    #                        f"{int(i2 + 1):5d}  {resnames[i2]:3s} {chains[i2]:1s} {int(resids[i2]):4d}    "
        ...
        for i2 in range(nresidues):
            ...
            if (over > 0 or cont > 0) and (i1 != i2):
                if over == 1 or (over == 0 and rcsu == 1):
                    ...
                    contacts.append((resids[i1], chains[i1], resids[i2], chains[i2]))
    return contacts
